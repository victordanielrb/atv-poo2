...
const criarClienteVazio = (): Cliente => ({
    nome: '',
    nomeSocial: '',
    dataNascimento: '',
...
const criarTelefoneVazio = (): Telefone => ({
    ddd: '',
    numero: ''
});
...
const validarCliente = (cliente: Cliente) => {
    ...
    if (!cliente.nome.trim()) erros.push('Nome é obrigatório');
    if (!cliente.dataNascimento) erros.push('Data de nascimento é obrigatória');
    if (cliente.telefones.length === 0) erros.push('Pelo menos um telefone é obrigatório');
...
const AnexarClienteDependente = ({
    ...
    const [dados, setDados] = useState<Cliente>(criarClienteVazio());
    
    const [titularSelecionado, setTitularSelecionado] = useState<string>('');
    const [telefoneAtual, setTelefoneAtual] = useState<Telefone>(criarTelefoneVazio());
    const [documentoAtual, setDocumentoAtual] = useState<Documento>(criarDocumentoVazio());const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        
        if (!titularSelecionado) {
            alert('Por favor, selecione um titular.');
            return;
        }        // Usar validação das utilities
        ...
        try {
            // Buscar o titular selecionado
            const titulares = await Instancia.buscarTitulares();
            const titular = titulares.find(t => t.nome === titularSelecionado);
            ...
            // Usar a função da Instancia para anexar dependente
            await Instancia.anexarDependente(novoDependente, titular._id!.toString());
            
            alert('Dependente anexado com sucesso!');
            onCadastrar(novoDependente, titularSelecionado);
            
            // Limpar formulário
            setDados(criarClienteVazio());
            setTelefoneAtual(criarTelefoneVazio());
            setDocumentoAtual(criarDocumentoVazio());
            setTitularSelecionado('');
        } catch (error) {
            console.error('Erro no processo de anexar dependente:', error);
